...
//
//    sheet->setCol(0, 10, 8);
//    sheet->setRow(rowNum+1,25);
//    sheet->setCol(1, 1, 40);
//    sheet->setCol(2, 2, 4);
//    sheet->setCol(3, 5, 14);
//    sheet->setRow(rowNum+6, 15);
//    sheet->setRow(rowNum+14, rowNum+15);
//
//    rowNum++;
//    sheet->writeStr(1, 0, L"中天廚房設備有限公司", boldFormat);
//    sheet->setMerge(rowNum, rowNum, 0, 5);
//    rowNum++;
//    sheet->writeStr(rowNum, 0, L"CHUNG TIN KITCHEN WARES COMPANY LIMITED", bold2Format);
//    sheet->setMerge(rowNum, rowNum, 0, 5);
//    rowNum++;
//    sheet->writeStr(rowNum, 0, L"香港九龍馬頭角道105號地下", normalFormat);
//    sheet->setMerge(rowNum, rowNum, 0, 5);
//    rowNum++;
//    sheet->writeStr(rowNum, 0, L"G/F 105 Ma Tau Kok Road Kowloon Hong Kong", normalFormat);
//    sheet->setMerge(rowNum, rowNum, 0, 5);
//    rowNum++;
//    sheet->writeStr(rowNum, 0, L"電話: (852) 2363 1482  傳真: (852) 2766 1415", normalFormat);
//    sheet->setMerge(rowNum, rowNum, 0, 5);
//    rowNum++;
//    sheet->writeStr(rowNum, 0, L"機電處註冊氣體工程承辦商號碼 RGC NO.-(682-06)", normalFormat);
//    sheet->setMerge(rowNum, rowNum, 0, 5);
//    rowNum++;
//    sheet->writeFormula(rowNum, 0, L"HYPERLINK(\"E-mail:chungtinkitchen@netvigator.com\")",linkFormat);
//    sheet->setMerge(rowNum, rowNum, 0, 5);
//    rowNum++;
//    sheet->writeStr(rowNum, 0, L"報價單", underlineFormat);
//    sheet->setMerge(rowNum, rowNum, 0, 5);
//    rowNum++;
//    sheet->writeStr(rowNum, 0, L"客戶名稱:", normalFormatRight);
//    std::string name = details->getCustomerName();
//    std::wstring name2(name.begin(), name.end());
//    sheet->writeStr(rowNum, 1, name2.c_str(), normalFormatLeft);
//    sheet->writeStr(rowNum, 4, L"報價單號碼:", normalFormatRight);
//    std::string quotationID = details->getQuotationID();
//    std::wstring quotationID2(quotationID.begin(), quotationID.end());
//    sheet->writeStr(rowNum, 5, quotationID2.c_str(), normalFormatLeft);
//    rowNum++;
//    sheet->writeStr(rowNum, 0, L"客戶地址:", normalFormatRight);
//    std::string addr = details->getCustomerAddr();
//    std::wstring addr2(addr.begin(), addr.end());
//    sheet->writeStr(rowNum, 1, addr2.c_str(), normalFormatLeft);
//    sheet->writeStr(rowNum, 4, L"Staff:", normalFormatRight);
//    std::string staffName = details->getStaffName();
//    std::wstring staffName2(staffName.begin(), staffName.end());
//    sheet->writeStr(rowNum,5, staffName2.c_str(), normalFormatLeft);
//    rowNum++;
//    sheet->writeStr(rowNum, 0, L"負責人:", normalFormatRight);
//    std::string supervisor = details->getSupervisor();
//    std::wstring supervisor2(supervisor.begin(), supervisor.end());
//    sheet->writeStr(rowNum, 1, staffName2.c_str(), normalFormatLeft);
//    sheet->writeStr(rowNum, 4, L"日期:", normalFormatRight);
//    std::string date = HELPFUNCTIONS_H::getCurrentDate();
//    std::wstring date2(date.begin(), date.end());
//    sheet->writeStr(rowNum, 5, date2.c_str(), normalFormatLeft);
//    rowNum++;
//    sheet->writeStr(rowNum, 0, L"電話:", normalFormatRight);
//    std::string phone = std::to_string(details->getCustomerTel());
//    std::wstring phone2(phone.begin(), phone.end());
//    sheet->writeStr(rowNum, 1, phone2.c_str(), normalFormatLeft);
//    rowNum++;
//    sheet->writeStr(rowNum, 0, L"客戶編號:", normalFormatRight);
//    std::string cusId = std::to_string(details->getCustomerID());
//    std::wstring cusID2(cusId.begin(), cusId.end());
//    sheet->writeStr(rowNum, 1, cusID2.c_str(), normalFormatLeft);
//    rowNum++;
//    sheet->writeStr(rowNum, 0, L"項目:", cubicFormat);
//    sheet->writeStr(rowNum, 1, L"內容:", cubicFormat);
//    sheet->writeStr(rowNum, 2, L"數量", cubicFormat);
//    sheet->writeStr(rowNum, 3, L"單價", cubicFormat);
//    sheet->writeStr(rowNum, 4, L"金額", cubicFormat);
//    sheet->writeStr(rowNum, 5, L"兆焦耳(MJ/HR)", cubicFormat);
//
//
//    rowNum++;
//
...
//            sheet->writeNum(rowNum, columnNum, discount, discountFormat);
//            columnNum++;
//            sheet->writeNum(rowNum, columnNum, discount, discountFormat);
//            totalDiscount += discount;
//        }
//        sheet->writeNum(++rowNum, 5, i);
//        sheet->writeNum(rowNum, 6, entries.size());
//        if (i == entries.size() - 1)
//            endOfFile(total, totalDiscount, totalMR, &book, &sheet, rowNum, details->getfileName());
//            return true;
//        i++;
//    }
//    index += 6;
//    rowNum += 5;
//    appendToFile(&book, &sheet, details, entries, index, total, totalDiscount, totalMR, rowNum);
//    return true;
//}
//
//bool endOfFile(int total, int totalDiscount, float totalMR, Book** newBook, Sheet** newSheet, int rowNum, std::string fileName)
//{
//    Book* book = *newBook;
//    Sheet* sheet = *newSheet;
//
...
//    rowNum++;
//    sheet->writeStr(rowNum, 3, L"合共兆焦耳(MJ/HR)", bold2FormatRight);
//    sheet->writeNum(rowNum, 4, totalMR, MJBoldFormat);
//    rowNum += 2;
...
//    sheet->writeStr(rowNum, 4, L" ", cubicFormat);
//
//    std::string path = HELPFUNCTIONS_H::get_desktop_path() + "/" + fileName;
...
bool appendToFile(customerDetails* details, std::vector<entry*> entries)
{
    ...
    Font* linkFont = book->addFont();
    linkFont->setColor(COLOR_BLUE);
    linkFont->setUnderline(UNDERLINE_SINGLEACC);
    ...
    Font* underlineFont = book->addFont();
    underlineFont->setUnderline(UNDERLINE_SINGLEACC);
    underlineFont->setBold(true);
    underlineFont->setSize(10);
